...
int is_expression_in_line(char *line, Grep_args *grep_args)
{
	...
	if (grep_args->x_arg) {
		match_found = is_match(&modified_line[0], grep_args->expression_to_match, grep_args); // look for an exact match just 
	}
	else {
		for (i = 0; i < strlen(modified_line); i++) {
			if (is_match(&modified_line[i], grep_args->expression_to_match, grep_args) == 1) { // look for a match in all possible positions
				match_found = 1;
				break;
			}
		}
...
int is_match_for_regular_expression(char *expression_to_search, char *line, Grep_args *grep_args) {
	if (expression_to_search[0] == '.') {
		return is_match__with_period(expression_to_search, line, grep_args);
	}
...
int is_match__with_period(char *expression_to_search, char *line, Grep_args *grep_args) {
	return is_match(&line[1], &expression_to_search[1], grep_args);
}
...
int is_match_with_round_brackets(char *expression_to_search, char *line, Grep_args *grep_args) {
	...
	int original_x_arg = grep_args->x_arg; // temporarily assign x_arg = 0 to check if the bracket expression is a prefix of the given line
	grep_args->x_arg = 0;
